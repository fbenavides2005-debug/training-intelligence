...
function getBodyMeasurement(accessToken) {
  return whoopFetch(accessToken, '/v2/user/body_measurement');
}

// --- Cycles ---

function getCycles(accessToken, { limit, start, end, nextToken } = {}) {
  return whoopFetch(accessToken, '/v2/cycle', { limit, start, end, nextToken });
}

function getCycleById(accessToken, cycleId) {
  return whoopFetch(accessToken, `/v2/cycle/${cycleId}`);
}

// --- Recovery ---

function getRecoveryCollection(accessToken, { limit, start, end, nextToken } = {}) {
  return whoopFetch(accessToken, '/v2/recovery', { limit, start, end, nextToken });
}

function getRecoveryForCycle(accessToken, cycleId) {
  return whoopFetch(accessToken, `/v2/cycle/${cycleId}/recovery`);
}

// --- Sleep ---

function getSleepCollection(accessToken, { limit, start, end, nextToken } = {}) {
  return whoopFetch(accessToken, '/v2/activity/sleep', { limit, start, end, nextToken });
}

function getSleepById(accessToken, sleepId) {
  return whoopFetch(accessToken, `/v2/activity/sleep/${sleepId}`);
}

// --- Workouts ---

function getWorkoutCollection(accessToken, { limit, start, end, nextToken } = {}) {
  return whoopFetch(accessToken, '/v2/activity/workout', { limit, start, end, nextToken });
}

function getWorkoutById(accessToken, workoutId) {
  return whoopFetch(accessToken, `/v2/activity/workout/${workoutId}`);
}
